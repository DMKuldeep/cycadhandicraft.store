-- ═══════════════════════════════════════════════════════════════════
-- CYCAD HANDICRAFTS — RUN THIS ENTIRE FILE ONCE IN SUPABASE SQL EDITOR
-- Do NOT run seed.sql alone. This file = schema + all dummy data.
-- ═══════════════════════════════════════════════════════════════════
...
CREATE POLICY "Active products are viewable by everyone" ON products FOR SELECT USING (is_active = true OR is_admin());
...
CREATE POLICY "Admins can view enquiries" ON enquiries FOR SELECT USING (is_admin());
CREATE POLICY "Admins can update enquiries" ON enquiries FOR UPDATE USING (is_admin());
CREATE POLICY "Admins can delete enquiries" ON enquiries FOR DELETE USING (is_admin());
...
CREATE POLICY "Admins can insert page content" ON page_content FOR INSERT WITH CHECK (is_admin());
CREATE POLICY "Admins can update page content" ON page_content FOR UPDATE USING (is_admin());
...
CREATE POLICY "Admins can update product images" ON storage.objects FOR UPDATE USING (bucket_id = 'product-images' AND is_admin());
CREATE POLICY "Admins can delete product images" ON storage.objects FOR DELETE USING (bucket_id = 'product-images' AND is_admin());

-- ═══════════════════════════════════════════════════════════════════
-- PART 7: SEED DATA (categories, products, pages)
-- Tables above MUST succeed before this section runs.
-- ═══════════════════════════════════════════════════════════════════
...
-- ─── VERIFY (should show 6, 18, 5) ───
SELECT 'categories' AS t, COUNT(*) FROM categories UNION ALL SELECT 'products', COUNT(*) FROM products UNION ALL SELECT 'page_content', COUNT(*) FROM page_content;
